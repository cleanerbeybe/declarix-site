...
const burden = {
  ...
  description:
    'Explore the HMRC/Ipsos 2024–2025 survey of 460 declarants, published in 2026. Active-work time and internal cost estimates, not elapsed time or product results.',
  h1: 'What 460 declarants reported about active work.',
  eyebrow: 'HMRC RESEARCH EXPLORER · 460 DECLARANTS · SOURCE-FAITHFUL',
  standfirst:
    'HMRC’s Wave 2 study reports 19–28 minutes of active work and £5–£7 estimated internal wage cost across typical processes. These are trimmed means, not elapsed clearance time, broker fees or Declarix results.',
  ref: 'PUBLIC STUDY 02 · HMRC / IPSOS',
  stamp: '460\nVOICES',
  reviewedOn: '2026-09-16',
  sources: [
    source('Customs Administrative Burden Wave 2', 'HM Revenue & Customs · GOV.UK', lineage.hmrcUrl, '16 SEPTEMBER 2026'),
    source('Wave 2 executive summary', 'HM Revenue & Customs · GOV.UK', 'https://www.gov.uk/government/publications/customs-administrative-burden-wave-2/customs-administrative-burden-wave-2-executive-summary', '16 SEPTEMBER 2026'),
    source('Full Wave 2 report (ODT)', 'HM Revenue & Customs · GOV.UK', lineage.hmrcFullUrl, '16 SEPTEMBER 2026'),
  ],
}
...
function jsonLd(route, site) {
  ...
  if (route.kind === 'burden-report') {
    graph.push({
      ...
      description: lineage.hmrcNotice,
      temporalCoverage: '2024-10-21/2025-01-17',
      measurementTechnique: lineage.hmrcMethod,
...
function burdenSvg() { return burdenLineageSvg() }
...
function renderBurden(route) {
  ...
  return `<section class="research-notice" aria-label="Research method and reuse limits"><strong>HMRC / IPSOS HISTORICAL STUDY</strong><p>${lineage.hmrcMethod}</p><p>${lineage.hmrcNotice}</p></section><section class="burden-read"><span>THE 90-SECOND READ</span><h2>Process burden was measured in minutes and internal cost—not software outcomes.</h2><p>HMRC commissioned Ipsos to survey 460 declarants. The published weighted split was 70% customs intermediaries and 30% traders. Fieldwork ran from 21 October 2024 to 17 January 2025. Raw group bases were 366 intermediaries and 94 traders; the percentages are weighted, not raw counts. The 15 qualitative interviews were a separate strand. Time estimates exclude waiting for other businesses. Cost estimates combine activity time and wage data, with extreme values trimmed. The study is not directly comparable to the 2023 wave.</p></section><section class="burden-metrics" aria-label="Headline findings"><div><span>SURVEY</span><strong>460</strong><small>declarants</small></div><div><span>TYPICAL PROCESS</span><strong>19–28</strong><small>active-work minutes; trimmed means</small></div><div><span>INTERNAL COST</span><strong>£5–£7</strong><small>estimated wage cost; trimmed means</small></div><div><span>AUTOMATED DATA ENTRY</span><strong>≈70%</strong><small>said it would help across process types</small></div></section><section class="burden-table" aria-labelledby="burden-table-heading"><header><span>SOURCE-FAITHFUL RANGE</span><h2 id="burden-table-heading">The endpoints HMRC published.</h2><p>Percentages and ranges stay as published. Weighted percentages are not converted into respondent counts.</p></header><div tabindex="0" role="region" aria-label="HMRC burden findings table"><table><caption>Headline Customs Administrative Burden Wave 2 findings</caption><thead><tr><th>Finding</th><th>Published result</th><th>Meaning</th></tr></thead><tbody><tr><th>Declarant mix</th><td>70% intermediaries / 30% traders</td><td>Published weighted split; n=460 study</td></tr><tr><th>Typical-process time</th><td>19–28 active-work minutes</td><td>Trimmed means; excludes waiting; typical means no major issues</td></tr><tr><th>Internal process cost</th><td>£5–£7</td><td>Estimated internal wage cost, not a broker fee; bases vary</td></tr><tr><th>Automated data entry</th><td>Approximately 70%</td><td>Would help across process types</td></tr><tr><th>No change / already efficient</th><td>At most 1 in 5</td><td>Upper bound, not an exact 20%</td></tr></tbody></table></div></section><section class="research-separation"><div><span>HMRC / IPSOS STUDY</span><strong>460 declarants</strong><p>Administrative burden across six customs processes. Published 26 February 2026.</p></div><div><span>DECLARIX SIGNAL REPORT</span><strong>402 qualified accounts</strong><p>Separate agent-assisted July research classifications. They are not survey evidence, market size or product outcomes.</p></div><div><span>DECLARIX WORKED MODEL</span><strong>Your inputs</strong><p>The desk calculator is a scenario, not a result from either research population.</p></div></section><div class="download-row">${renderDownload(route, csv, 'Source-faithful summary data')}${renderDownload(route, svg, 'Press-ready findings chart')}</div><section class="authority-bridge"><span>RUN YOUR OWN DESK NUMBERS</span><h2>Replace the study range with your workflow.</h2><p>Use your volume, preparation time, rework and loaded clerk cost. No Declarix rate is assumed.</p><a data-authority-related="desk_calculator" href="/tools/customs-declaration-cost-calculator/">OPEN THE FREE COST CALCULATOR →</a></section>`
}
